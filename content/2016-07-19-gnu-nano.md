...
To circumvent this issue, we can press the {{ kbd(keys=[["Esc", "W"]]) }} {{ kbd(keys=["Esc"]) }} key twice and then the key that appears after the `^`. For instance, to make use of the `^W`&nbsp;`Where Is` shortcut, we would use the following key sequence, pressing the keys one after the other:<br />{{ kbd(keys=["Esc", "Esc", "W"], separator="&rarr;") }}
